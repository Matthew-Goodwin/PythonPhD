...
def loadData(dataLocation,npy,output,variableName,A_scan_num,B_scan_num,padSize):
    ...
    if check==0:
        ...
        Int, Ret,Ch0Complex,Ch1Complex = pytdms.Scan_processing_3D(ch0,ch1,padSize)
...
def checkDirectory(dataLocation,npy,output,variableName,autoProcess=True):
    if os.path.isdir(dataLocation + npy + '\\' + variableName):
        if os.path.isfile(dataLocation + npy + '\\' + variableName + '\Int' +   '.npy') and os.path.isfile(dataLocation + npy + '\\' + variableName + '\Ret' +   '.npy') and os.path.isfile(dataLocation + npy + '\\' + variableName + '\Ch0Complex' +   '.npy') and os.path.isfile(dataLocation + npy + '\\' + variableName + '\Ch1Complex' +   '.npy'):
